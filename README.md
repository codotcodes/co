...
Enrollment saves a separate machine credential in the owner-only CLI config for the exact API used at enrollment. `co machine list` shows enrolled machine IDs without credentials. Start `co machine run MACHINE_ID` in the foreground; it sends a heartbeat and polls for assigned jobs every 30 seconds. A registered machine is shown as offline after 90 seconds without a heartbeat. Keep the same `CO_API_URL` when using a non-production API. Rootless Podman and a locally available image are required for execution. The default pinned Alpine image supports shell steps; set `CO_MACHINE_IMAGE` to another already provisioned image pinned by `@sha256:` for builds needing other tools. Checkout uses a short-lived pull-only token outside the container. Steps have no network or host credentials, a 10-minute per-step and 30-minute total limit, and return at most 1 KiB of output each. A disconnected or cancelled run cannot report success after its lease expires.
